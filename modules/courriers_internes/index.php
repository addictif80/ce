<?php
$pageTitle = 'Générateur de courriers internes';
...
$db->exec("CREATE TABLE IF NOT EXISTS courriers_internes (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    civilite_dest VARCHAR(20) DEFAULT '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    $stmt = $db->prepare("DELETE FROM courriers_internes WHERE id = ? AND user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_template') {
    ...
    $stmt = $db->prepare("INSERT INTO modeles_courriers_internes (user_id, nom_modele, objet, corps, variables, approved, approved_by) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_template') {
    $id = (int)$_POST['tpl_id'];
    if ($isUserAdmin) {
        $stmt = $db->prepare("DELETE FROM modeles_courriers_internes WHERE id = ?");
        $stmt->execute([$id]);
    } else {
        $stmt = $db->prepare("DELETE FROM modeles_courriers_internes WHERE id = ? AND user_id = ?");
...
// Liste courriers
$stmt = $db->prepare("SELECT * FROM courriers_internes WHERE user_id = ? ORDER BY date_courrier DESC");
$stmt->execute([$userId]);
...
$stmt = $db->prepare("SELECT m.*, u.nom AS author_nom, u.prenom AS author_prenom
    FROM modeles_courriers_internes m
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-4 d-flex align-items-center">
        <button class="btn btn-ce" data-bs-toggle="modal" data-bs-target="#addModal"><i class="fas fa-plus"></i> Nouveau courrier interne</button>
    </div>
</div>
...
<div class="data-table-container">
    <div class="data-table-header">
        <h3>Tous les courriers internes</h3>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-plus"></i> Nouveau courrier interne</h5>
                        ...
                        <div class="col-12">
                            <div class="variables-section">
                                ...
                                <div class="variables-help">
                                    Cliquez sur une variable pour l'insérer dans le corps du courrier. Les variables fixes sont remplies automatiquement depuis les champs du client.
                                </div>
...
<div class="modal fade modal-fullscreen-custom" id="detailModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-envelope-open-text"></i> Détail du courrier interne</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade modal-fullscreen-custom" id="editModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-edit"></i> Modifier le courrier interne</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
