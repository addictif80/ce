...
CREATE TABLE IF NOT EXISTS `mobilites` (
  ...
  -- Comptes à transférer (Mobiliz)
  `mobiliz_compte_cdd` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_livret_a` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_livret_b` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_lep` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_ldds` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_assurance_vie` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_pea` TINYINT(1) DEFAULT 0,
  `mobiliz_compte_parts_sociales` TINYINT(1) DEFAULT 0,
  -- Rendez-vous Mobiliz
  `mobiliz_rdv_date` DATE DEFAULT NULL,
  `mobiliz_rdv_heure` VARCHAR(5) DEFAULT NULL,
  `mobiliz_rdv_honore` TINYINT(1) DEFAULT 0,
  `notes` TEXT DEFAULT NULL,
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE CASCADE
